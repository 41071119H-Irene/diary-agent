...
def generate_mood_trend_plot(user_id, user_entries):
    ...
    # SnowNLP分析心情小語，映射成1~10分
    user_entries["心情小語分析"] = user_entries["心情小語"].apply(lambda text: SnowNLP(text).sentiments * 9 + 1)

    # 根據日記數量選擇作圖方式
    if len(user_entries) > 30:
        # 超過30筆，改每週彙整
        user_entries.set_index("日期", inplace=True)
        weekly_data = user_entries.resample('W-SUN').mean(numeric_only=True).reset_index()

        plot_data = weekly_data
        x_col = "日期"
        y_cols = ["心情指數", "心情小語分析"]
        title = f"用戶 {user_id} 的每週心情趨勢圖"
    else:
        # 少於30筆，維持逐日繪圖
        plot_data = user_entries
        x_col = "日期"
        y_cols = ["心情指數", "心情小語分析"]
        title = f"用戶 {user_id} 的心情趨勢圖"

    avg_recorded = plot_data["心情指數"].mean()
    avg_snownlp = plot_data["心情小語分析"].mean()

    plt.figure(figsize=(12, 6))
    sns.lineplot(x=x_col, y="心情指數", data=plot_data, marker="o", label="用戶心情紀錄", color="blue", errorbar=None)
    sns.lineplot(x=x_col, y="心情小語分析", data=plot_data, marker="o", label="SnowNLP 心情分析", color="red", errorbar=None)
