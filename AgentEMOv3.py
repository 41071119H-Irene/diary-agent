...
def generate_mood_trend_plot(user_id, user_entries):
    """
    生成用戶的心情走向趨勢圖，並加入平均心情指數線，將圖片存入 `moodtrend/` 資料夾。
    """
    # 確保 moodtrend 資料夾存在
    output_dir = "moodtrend"
    ...
    plt.plot(user_entries["日期"], user_entries["心情指數"], marker='o', linestyle='-', label="每日心情指數", color="blue")
    ...
    plt.xlabel("日期")
    plt.ylabel("心情指數")
    plt.title(f"用戶 {user_id} 的心情走向趨勢")
